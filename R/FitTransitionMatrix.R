...
#' @param q number of param
#' @param index.matrix
#' @param scale scaling of the transition matrix in and out parameters.
#'
#' @return Transition matrix in the correct form for Felsensteins pruning algorithm.
#' @export
#'
#' @examples
makeQfreeX<-function(m,q,index.matrix,scale){ #For binary optimization for COVID
  Q<-matrix(0,m,m)
  Q[]<-c(0,q)[index.matrix+1]
  diag(Q)<-0
  Q[2,1] = scale*Q[1,2]
...
#'
#' @examples
FitTransitionFreeX<-function(tree,x,model=NA, scaling_factor = 10){
  ...
  fit<-nlminb(q.init,function(p) lik(makeQfreeX(m,p,index.matrix,scaling_factor),pi=pi),lower=rep(0,k),upper=rep(1e50,k),hessian = T)
